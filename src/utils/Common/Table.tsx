    ...
    <>
      <div className="overflow-x-auto shadow-lg rounded-md mb-3">
        <table className="min-w-full w-full text-left text-sm font-light table-auto">
          ...
          <tbody className="text-sm divide-y divide-gray-300">
            {(tname === "Pending Table" || tname === "Review Table") &&
              tData?.map((row, index) => (
                <tr
                  key={index}
            ...
            {tname === "Pending Amount Table" &&
              tData?.map((row, index) => (
                <tr
                  key={index}
                  className="border-bt ransition duration-300 ease-in-out"
                >
            ...
            {(tname === "Rejected Master Agreement Table" ||
              tname === "Pending Master Agreement Table") &&
              tData?.map((row, index) => (
                <tr
                  key={index}
                  className="border-b transition duration-300 ease-in-out hover:bg-green-2"
                >
                  ...
                  {row?.actions && (
                    <td className="whitespace-nowrap px-6 text-left font-medium font-Roboto text-silver__color">
                      <select className="block appearance-none w-full border text-primary py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-borderColor">
            ...
            {tname === "Approved Master Agreement Table" &&
              tData?.map((row, index) => (
                <tr
                  key={index}
                  className="border-b transition duration-300 ease-in-out hover:bg-green-2"
            ...
            {tname === "Compliance Table" &&
              tData?.map((row, index) => (
                <tr
                  key={index}
                  className="border-b py-3 transition duration-300 ease-in-out hover:bg-green-2"
                >
                  <td className="whitespace-nowrap px-6 text-left font-medium font-Roboto text-silver__color">
                    {row?.selectBank}
                  </td>
      ...
      {show && (
        <div className="flex items-center justify-end">
          <Button
            callBack={() => updateModal("Add Payment")}
            title="Add Payment"
          />
        </div>
      )}
      {modal.state === "Upload file" && (
        <CommonPopUp
          title="Upload File and Approve"
          component={<UploadFile />}
          closeModal={closeModal}
        />
      )}
      {modal.state === "Add Payment" && (
        <CommonPopUp
          title="Add Payment"
